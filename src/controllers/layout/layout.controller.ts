...
export default {
  ...
  createLayout: castAsynError(async (req:Request, res:Response, next:NextFunction) => {
    try {
      const {type}  = req.body;
      ...
      if (type === 'Banner') {
        ...
        const bannerImage = await uploadToCloud(req.file, 'layout')
  ...
  editlayout: castAsynError( async (req:Request, res:Response, next:NextFunction) => {
    try {
  ...
  getLayoutByType: castAsynError(async (req:Request, res:Response, next:NextFunction) => {
    try {
